  ...
  const SingIn =(
    <div className="mt-16">
      <form className='flex flex-col gap-8'>
        ...
        <button className='w-full border mt-4 py-2 rounded-lg'>SingIn</button>
      </form>
    </div>
  )
        ...
        <div className="flex justify-center items-center">
            <div className=" flex- flex-col gap-7">
              <div className="border-b py-4 flex gap-8">
                <button onClick={() => setIsSingIn(false)} className={`font-semibold text-slate-700 ${isSingIn ? 'opacity-60' : ''}`}>SignUp</button>
                <button onClick={() => setIsSingIn(true)} className={`font-semibold text-slate-700 ${isSingIn ? '' : 'opacity-60'}`}>SingIn</button>
              </div>
